...
void send(const Message &message, int destination_rank) {
...
void sendAll(const Message &message, int source_rank, int world_size) {
...
std::optional<MPI_Status> checkForMessage(int source) {
  int request_completed;
  MPI_Status status;
  MPI_Iprobe(source, MPI_ANY_TAG, MPI_COMM_WORLD, &request_completed,
             &status);
...
json waitForResponse(int source) {
  std::optional<MPI_Status> statusOpt;
  do
  {
    statusOpt = checkForMessage(source);
  } while (!statusOpt.has_value());
  
  return recv(statusOpt.value());
}