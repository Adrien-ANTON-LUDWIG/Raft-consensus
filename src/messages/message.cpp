...
std::string Message::UUIDToStr() const { return uuids::to_string(m_uuid); }

Message::Type Message::getType() const { return m_type; }

Message::Type Message::getType(const json& data) { return Type(data["type"]); }

int Message::getOriginId() const { return m_originId; }
...
bool Message::isCMD(int type) {
  return type == CMD_LOAD || type == CMD_LIST || type == CMD_DELETE ||
         type == CMD_APPEND;
}
}  // namespace MessageNS
